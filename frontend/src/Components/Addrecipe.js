  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.React_App_Backend_Url}/api/v1/user/addrecipe/` +
          currentuserid,
        userEnteredRecipe,
        { headers }
      );
      ...
      props.setImageData((prev) => ({
        ...prev,
        [userEnteredRecipe.image]: uploadedImageUrl,
      }));
      const addingredientsResponse = await axios.post(
        `${process.env.React_App_Backend_Url}/api/v1/user/addingredients/` +
          response.data.text,
