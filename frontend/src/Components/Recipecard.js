...
function Editform(props) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.put(
        `${process.env.React_App_Backend_Url}/api/v1/user/editrecipe/` +
          props.userid,
        userEnteredRecipe,
        { headers }
      );
      ...
      const editingredientsResponse = await axios.put(
        `${process.env.React_App_Backend_Url}/api/v1/user/editingredients/` +
          response.data.text,
  ...
  const handleDeleterecipe = async (event) => {
    const response = await axios.post(
      `${process.env.React_App_Backend_Url}/api/v1/user/delete`,
      { id: props.details.id },
      { headers }
    );
